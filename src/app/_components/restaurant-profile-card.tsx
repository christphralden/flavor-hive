...
interface RestaurantProfileCardInterface{
    restaurant:PocketbaseTyped<Restaurant>
}
export default async function RestaurantProfileCard({restaurant}:RestaurantProfileCardInterface) {
    const headerImage = pb.files.getUrl(restaurant.record, (Array.isArray(restaurant.data.images) ? restaurant.data.images[0] as string : "") , {'thumb': '0x300'});
    const coverImage = pb.files.getUrl(restaurant.record, restaurant.data.cover as string , {'thumb': '0x300'});
    return (
        <Link className='m-0' href={`/restaurant/${restaurant.data.id}`}>
            <Card className='w-[250px] h-[350px] lg:w-[350px] lg:h-[500px] overflow-clip bg-white rounded-lg'>
                <CardHeader className='h-[40%] w-full bg-black p-0 relative '>
                    ...
                    <Avatar className='lg:w-20 lg:h-20 w-16 h-16 absolute bottom-0 lg:left-8 left-4 translate-y-[50%]' >
                        <AvatarImage className='w-full h-full object-cover' src={coverImage} alt={`@${restaurant.data.name}`}/>
                    ...
                    <section className='w-full flex flex-col gap-2 lg:gap-4 '>
                        <div className='w-full flex flex-col gap-1 text-base lg:text-lg'>
                            <div className='w-full flex justify-start items-start font-medium leading-5 '>
                                <h1 className='w-[70%] text-wrap line-clamp-2'>{restaurant.data.name}</h1>
                            ...
                            <div className='w-full'>
                                <p className='text-gray-500 text-xs lg:text-sm font-normal line-clamp-1'>{restaurant.data.location}</p>
                            </div>
                            <Separator className='lg:mt-2 mt-1'/>
                        </div>
                        <div className='w-full'>
                            <p className='text-gray-500  line-clamp-3 text-xs lg:text-sm'>{restaurant.data.description}</p>
                        </div>
                    </section>
                    <section className='hidden  gap-2 lg:flex'>
                        {
                            restaurant.data.keywords?.tags.map((tag, i)=>{
                                const limit = 2
                                if(i==limit)return(
                                    <Badge key={i} variant={'outline'} className='p-2 px-4 text-xs lg:text-xs'>+{(restaurant.data.keywords?.tags.length||0) - limit}</Badge>
                                )
                                if(i<limit)return(
