...
export async function getRestaurantReviewsAmount({
  restaurantId,
}: {
  restaurantId: string;
}): Promise<number> {
...
export async function createRestaurantReview({
  ...
  try {
    ...
    if (!(reviewRecord.rating >= 0 && reviewRecord.rating <= 5))
      throw new Error("Rating must be between 0 and 5.");

    await pb.collection(PB_KEYS.REVIEWS).create(reviewRecord);

    const restaurantReviewAmount = await getRestaurantReviewsAmount({
      restaurantId: reviewRecord.restaurant,
    });
    const prevRestaurantReviewAmount = restaurantReviewAmount - 1;
    const restaurant = await getRestaurant({
      restaurantId: reviewRecord.restaurant,
    });

    const newAverageRating =
      (restaurant.cachedRating * prevRestaurantReviewAmount +
        reviewRecord.rating) /
    ...
    await pb
      ...
      .update(reviewRecord.restaurant, restaurant);
